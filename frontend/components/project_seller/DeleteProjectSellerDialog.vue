	...
	>
		<template #header>
			<div class="inline-flex items-center justify-center gap-2">
				<span class="font-bold text-xl">Delete Seller From Project?</span>
			</div>
		</template>
		<div>
			<span>
				This will delete seller
				<b>{{
					`${currentProjectSeller['slFirstName']} ${currentProjectSeller['slLastName']}`
				}}</b>
				from project
				<b>{{ currentProjectSeller['pjName'] }}</b>
				permanently!
			</span>
